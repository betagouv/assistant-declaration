...
async function setColorSchemeToBrowser(page: Page, colorScheme: 'light' | 'dark', timeToWaitAfterChangingThemeMs?: number) {
...
const config: TestRunnerConfig = {
  ...
  async preRender(page, context) {
    await Promise.all([
      setColorSchemeToBrowser(page, 'light'),
      injectAxe(page),
      // ---
  ...
  async postRender(page, context) {
    ...
    await setColorSchemeToBrowser(page, 'dark', timeToWaitAfterChangingThemeMs);
