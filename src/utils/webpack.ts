...
export function applyRawQueryParserOnStorybookCssModule(moduleRules: NonNullable<ModuleOptions['rules']>) {
...
export function applyRawQueryParserOnNextjsCssModule(moduleRules: NonNullable<ModuleOptions['rules']>) {
  ...
  for (const originalRule of moduleRules) {
    if (originalRule && typeof originalRule === 'object' && Array.isArray(originalRule.oneOf)) {
      for (const ruleItem of originalRule.oneOf) {
        if (ruleItem && ruleItem.test && styleRegex.test(ruleItem.test.toString())) {
