...
import { getEventDateCollection, getTicketCollection, getTicketingCollection } from '@ad/src/client/weezevent';
...
} from '@ad/src/models/entities/weezevent';
import { workaroundAssert as assert } from '@ad/src/utils/assert';

export class WeezeventTicketingSystemClient implements TicketingSystemClient {
  ...
  constructor(
    ...
    this.client = createClient(
      createConfig({
        baseUrl: 'https://api.weezevent.com/ticket/',
      })
    );
  }
  ...
  public async getEventsSeries(fromDate: Date, toDate?: Date): Promise<LiteEventSerieWrapperSchemaType[]> {
    ...
    await eachOfLimit(ticketingsToSynchronize, 1, async (ticketing) => {
      // Weezevent allows creating "ticketings" that are not directly linked to a live performance
      ...
      if (ticketing.eventDateList.length > 0) {
        ...
        for (const eventDate of eventDatesData['hydra:member']) {
          ...
          // Since the Weezevent logic differs from ours since it considers a category price per event date
        ...
        for (const ticket of ticketsData['hydra:member']) {
          ...
          if (ticket.ticketPrice === null) {
            if (ticket.imported) {
              ...
              throw new Error(`a ticket has no "ticketPrice" whereas it has not been manually imported weezevent, this should be investigated`);
            }
          }
