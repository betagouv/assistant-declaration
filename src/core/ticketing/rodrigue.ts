...
import { getApiTokenByUsername } from '@ad/src/client/rodrigue/auth';
...
export class RodrigueTicketingSystemClient implements TicketingSystemClient {
  protected readonly authClient: Client;
  protected readonly integrationClient: Client;
  protected langCode: 'en' = 'en';
  protected readonly maximumItemsPerPage: number = 200;

  constructor(
    private partnerName: string,
    private partnerSecretKey: string,
    private accessKey: string,
    private secretKey: string
  ) {
    this.authClient = createClient(
      createConfig({
        baseUrl: 'https://staging.rodrigue-ws.com/v1/authentication/api',
      })
    );
    this.integrationClient = createClient(
      createConfig({
        baseUrl: 'https://staging.rodrigue-ws.com/v1/external/api',
      })
    );
  }

  private getSignature(httpMethod: 'GET', url: string): string {
    const salt = this.partnerSecretKey;
    const data = `${httpMethod} ${url}`;

    const hmac = crypto.createHmac('sha512', Buffer.from(salt, 'utf8'));
  ...
  public async login(): Promise<string> {
    const authResult = await getApiTokenByUsername({
      client: this.authClient,
      headers: {
        Signature: this.getSignature('GET', `/api/token/${this.partnerName}`), // Did not succeed using properties `security+auth`, so falling back to raw headers
      },
      path: {
    ...
    assert(authResult.data?.authenticationToken);

    return authResult.data.authenticationToken;
  }
