...
- Développement (dans notre cas aussi assimilé à du "staging" / à de l'intégration) : https://assistant-declaration.incubateur.net

**La production est réservée à tout usage réel des éditeurs et des déclarants**, donc durant vos tests d'intégration de notre API nous vous demandons de tout faire sur l'environnement de "Développement".
...
<Alert
  ...
  description={`Les jetons d'accès n'ont pas d'expiration jusqu'à leur renouvellement. Depuis notre interface vous pouvez forcer la génération de nouveau accès, ou supprimer la billetterie en question pour que l'accès soit invalidé.`}
/>
...
Bien que vous puissiez proposer à vos clients d'utiliser la billetterie `Générique` pour récupérer des accès à configurer dans les paramètres de votre outil de billetterie, il peut être plus approprié d'avoir votre identité de marque comme cela est fait pour les éditeurs avec une intégration _vers leur API_.

Pour cela rien de plus simple, veuillez nous contacter pour que l'équipe technique ajuste la plateforme. Nous ferons en sorte de reprendre le connecteur `Générique`, mais à votre nom, avec votre logo et avec une documentation dédiée.
