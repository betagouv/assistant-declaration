...
export function UpdateEventSerieForm({ onSuccess, prefill }: UpdateEventSerieFormProps) {
  ...
  const {
    ...
    defaultValues: {
      eventSerieId: '',
      name: '',
      events: [], // Since one always required the default empty should never be visible by a real user
      ...prefill,
    },
  });
  ...
  const eventsWithErrorLogic = useMemo(() => {
    return fields
      .sort((a, b) => {
        return +a.startAt - +b.startAt; // Ascending
      })
      .map((field, index): RowForForm<typeof field, NonNullable<(typeof errors)['events']>[0]> => {
        return {
          index: index,
          data: field,
  ...
  return (
    <BaseForm handleSubmit={handleSubmit} onSubmit={onSubmit} control={control} ariaLabel="modifier un spectacle">
            ...
            <div className={fr.cx('fr-col-12', 'fr-py-6v')} style={{ display: 'flex' }}>
              <Button
                ...
                onClick={(event) => {
                  ...
                  append({
                    id: null, // Will act as one to create
                    startAt: new Date(),
                    endAt: null,
                  });
                }}
