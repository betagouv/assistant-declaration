...
export default function TableRow({ children, style: styleProps, _firstRow, _lastRow, ...rest }: TableRowProps) {
  ...
  const cells = useMemo(() => {
    ...
    const flattenChildren = (elem: typeof children) => {
      React.Children.forEach(elem, (child) => {
        ...
        if (child.type === React.Fragment) {
          flattenChildren((child.props as { children?: React.ReactNode }).children as any);
        } else {
          output.push(child);
        }
      });
    ...
    const remainingWeighting = 1 - weightings.reduce((acc: number, val) => acc + (val || 0), 0);
  ...
  return (
    <View
      ...
      style={[
        ...
        ...((Array.isArray(trStyle) ? trStyle : [trStyle]) as any),
        ...((Array.isArray(styleProps) ? styleProps : [styleProps]) as any),
      ]}
    >
      {cells}
    </View>
